import { useMemo, useState, useRef, useCallback } from 'react'
import { useNavigate, useSearchParams } from 'react-router'
import { useDatabase } from '@/hooks/useDatabase'
import { searchEntries } from '@/db/database'
import { Badge } from '@/components/ui/Badge'
import { EntryDetailSheet } from '@/components/entry/EntryDetailSheet'
import { toast } from '@/components/ui/Toaster'
import { AnimatePresence, motion } from 'framer-motion'
import { ArrowLeft, ChevronLeft, ChevronRight } from 'lucide-react'
import type { LogEntry } from '@/types'
import { getNoteTypeColor } from '@/types'

const TABS = [
  { key: 'all', label: 'All' },
  { key: 'Activity', label: 'Activities' },
  { key: 'Complaints', label: 'Complaints' },
  { key: 'Abnormality', label: 'Abnormalities' },
  { key: 'Resolved Complaint', label: 'Resolved' },
]

function formatDate(dateStr: string): string {
  try {
    const date = new Date(dateStr + 'T00:00:00')
    if (isNaN(date.getTime())) return dateStr
    return date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
  } catch {
    return dateStr
  }
}

export function ProfileDetail() {
  const navigate = useNavigate()
  const [searchParams] = useSearchParams()
  const { removeEntry, noteTypes } = useDatabase()

  const objectName = searchParams.get('object')
  const [activeTab, setActiveTab] = useState(0)
  const [selectedEntry, setSelectedEntry] = useState<LogEntry | null>(null)
  const [sheetOpen, setSheetOpen] = useState(false)

  // Swipe state
  const touchStartX = useRef<number | null>(null)
  const touchStartY = useRef<number | null>(null)
  const contentRef = useRef<HTMLDivElement>(null)

  // All entries for this object
  const allEntries = useMemo(() => {
    if (!objectName) return []
    const raw = searchEntries(objectName, 9999)
    const lowerQuery = objectName.toLowerCase()
    return raw.filter(e => e.object && e.object.toLowerCase() === lowerQuery)
  }, [objectName])

  // Metadata from first entry
  const meta = useMemo(() => {
    if (allEntries.length === 0) return null
    const first = allEntries[0]
    return { object: first.object, objectGroup: first.objectGroup, objectType: first.objectType }
  }, [allEntries])

  // Type counts
  const typeCounts = useMemo(() => {
    const counts: Record<string, number> = {}
    for (const entry of allEntries) {
      counts[entry.noteType] = (counts[entry.noteType] || 0) + 1
    }
    return counts
  }, [allEntries])

  // Filtered entries based on active tab
  const filteredEntries = useMemo(() => {
    const tab = TABS[activeTab]
    if (tab.key === 'all') return allEntries
    return allEntries.filter(e => e.noteType === tab.key)
  }, [allEntries, activeTab])

  // Group by date
  const grouped = useMemo(() => {
    const map = new Map<string, LogEntry[]>()
    const sorted = [...filteredEntries].sort(
      (a, b) => b.date.localeCompare(a.date) || b.id - a.id,
    )
    for (const entry of sorted) {
      if (!map.has(entry.date)) map.set(entry.date, [])
      map.get(entry.date)!.push(entry)
    }
    return map
  }, [filteredEntries])

  // Swipe handlers
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    touchStartX.current = e.touches[0].clientX
    touchStartY.current = e.touches[0].clientY
  }, [])

  const handleTouchEnd = useCallback((e: React.TouchEvent) => {
    if (touchStartX.current === null || touchStartY.current === null) return
    const deltaX = e.changedTouches[0].clientX - touchStartX.current
    const deltaY = e.changedTouches[0].clientY - touchStartY.current
    // Only trigger swipe if horizontal movement is dominant and significant
    if (Math.abs(deltaX) > 60 && Math.abs(deltaX) > Math.abs(deltaY) * 1.5) {
      if (deltaX < 0 && activeTab < TABS.length - 1) {
        setActiveTab(t => t + 1)
      } else if (deltaX > 0 && activeTab > 0) {
        setActiveTab(t => t - 1)
      }
    }
    touchStartX.current = null
    touchStartY.current = null
  }, [activeTab])

  if (!objectName) {
    return (
      <div className="flex flex-col h-full">
        <div className="px-4 pt-4 pb-24">
          <div className="py-16 text-center">
            <p className="text-text-muted text-base">No object specified</p>
            <p className="text-text-muted text-sm mt-1">
              Navigate from the Profiles tab to view an object's profile.
            </p>
          </div>
        </div>
      </div>
    )
  }

  return (
    <div className="flex flex-col h-full">
      {/* Back button + header */}
      <div className="px-4 pt-3 pb-2 space-y-3">
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-1.5 text-xs text-text-muted hover:text-text-secondary transition-colors"
        >
          <ArrowLeft size={14} />
          Back
        </button>

        {meta && (
          <div>
            <h1 className="text-xl font-bold text-text-primary">{meta.object}</h1>
            <div className="flex items-center gap-2 mt-0.5">
              {meta.objectType && (
                <span className="text-xs text-text-secondary font-medium bg-surface-2 px-2 py-0.5 rounded-md border border-border-subtle">
                  {meta.objectType}
                </span>
              )}
              {meta.objectGroup && (
                <span className="text-xs text-text-muted">{meta.objectGroup}</span>
              )}
            </div>
          </div>
        )}
      </div>

      {/* Summary stats */}
      <div className="px-4 pb-3">
        <div className="flex flex-wrap gap-2">
          <SummaryCard label="Total" count={allEntries.length} color="#A1A1AA" />
          {noteTypes.map((type) => {
            const count = typeCounts[type] || 0
            if (count === 0) return null
            return (
              <SummaryCard
                key={type}
                label={type}
                count={count}
                color={getNoteTypeColor(type)}
              />
            )
          })}
        </div>
      </div>

      {/* Swipeable tabs */}
      <div className="px-4 pb-2">
        <div className="relative flex bg-surface-2 rounded-xl p-1 border border-border-subtle overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
          {TABS.map((tab, i) => {
            const isActive = i === activeTab
            const count = tab.key === 'all'
              ? allEntries.length
              : typeCounts[tab.key] || 0
            return (
              <button
                key={tab.key}
                onClick={() => setActiveTab(i)}
                className="relative flex items-center justify-center gap-1 h-8 text-xs font-semibold rounded-lg flex-1 min-w-0 transition-colors z-10"
                style={{ color: isActive ? '#fff' : 'var(--color-text-muted)' }}
              >
                {isActive && (
                  <motion.div
                    layoutId="profile-tab-bg"
                    className="absolute inset-0 rounded-lg"
                    style={{
                      backgroundColor: tab.key === 'all'
                        ? 'var(--color-accent)'
                        : getNoteTypeColor(tab.key),
                    }}
                    transition={{ type: 'spring', stiffness: 500, damping: 35 }}
                  />
                )}
                <span className="relative truncate">{tab.label}</span>
                {count > 0 && (
                  <span className="relative text-[10px] opacity-70">{count}</span>
                )}
              </button>
            )
          })}
        </div>
        {/* Navigation arrows */}
        <div className="flex items-center justify-between mt-1">
          <button
            onClick={() => setActiveTab(t => Math.max(0, t - 1))}
            disabled={activeTab === 0}
            className="p-1 text-text-muted hover:text-text-secondary disabled:opacity-30 transition-opacity"
          >
            <ChevronLeft size={16} />
          </button>
          <span className="text-[10px] text-text-muted">
            Swipe or tap tabs to switch · {TABS[activeTab].label}
          </span>
          <button
            onClick={() => setActiveTab(t => Math.min(TABS.length - 1, t + 1))}
            disabled={activeTab === TABS.length - 1}
            className="p-1 text-text-muted hover:text-text-secondary disabled:opacity-30 transition-opacity"
          >
            <ChevronRight size={16} />
          </button>
        </div>
      </div>

      {/* Content area with swipe */}
      <div
        ref={contentRef}
        className="flex-1 overflow-y-auto px-4 pb-24"
        onTouchStart={handleTouchStart}
        onTouchEnd={handleTouchEnd}
      >
        <AnimatePresence mode="wait">
          <motion.div
            key={activeTab}
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -20 }}
            transition={{ duration: 0.15 }}
          >
            {filteredEntries.length === 0 ? (
              <div className="py-12 text-center">
                <p className="text-text-muted text-sm">No {TABS[activeTab].label.toLowerCase()} found</p>
                <p className="text-text-muted text-xs mt-1">
                  This object has no {TABS[activeTab].label.toLowerCase()} yet.
                </p>
              </div>
            ) : (
              <div className="space-y-5">
                {Array.from(grouped.entries()).map(([date, dateEntries]) => (
                  <div key={date}>
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-xs font-semibold text-text-secondary uppercase tracking-wider">
                        {formatDate(date)}
                      </h3>
                      <span className="text-[10px] text-text-muted font-medium bg-surface-2 px-1.5 py-0.5 rounded">
                        {dateEntries.length}
                      </span>
                    </div>
                    <div className="space-y-2">
                      <AnimatePresence>
                        {dateEntries.map((entry) => (
                          <EntryCard
                            key={entry.id}
                            entry={entry}
                            onClick={() => {
                              setSelectedEntry(entry)
                              setSheetOpen(true)
                            }}
                          />
                        ))}
                      </AnimatePresence>
                    </div>
                  </div>
                ))}
              </div>
            )}
          </motion.div>
        </AnimatePresence>
      </div>

      <EntryDetailSheet
        entry={selectedEntry}
        isOpen={sheetOpen}
        onClose={() => setSheetOpen(false)}
        onEdit={(entry) => {
          setSheetOpen(false)
          navigate(`/new?edit=${entry.id}`)
        }}
        onDelete={(id) => {
          removeEntry(id)
          setSheetOpen(false)
          toast('Entry deleted')
        }}
        onDuplicate={(entry) => {
          setSheetOpen(false)
          navigate(`/new?duplicate=${entry.id}`)
        }}
      />
    </div>
  )
}

/* ─── Summary card ─── */

function SummaryCard({
  label,
  count,
  color,
}: {
  label: string
  count: number
  color: string
}) {
  return (
    <div className="glass rounded-xl p-3 min-w-0">
      <div className="flex items-center gap-2 mb-1">
        <span
          className="w-2 h-2 rounded-full flex-shrink-0"
          style={{ backgroundColor: color }}
        />
        <span className="text-xs text-text-muted truncate">{label}</span>
      </div>
      <p className="text-xl font-bold text-text-primary">{count}</p>
    </div>
  )
}

/* ─── Entry card ─── */

function EntryCard({
  entry,
  onClick,
}: {
  entry: LogEntry
  onClick: () => void
}) {
  const truncatedNote =
    entry.note.length > 120
      ? entry.note.slice(0, 120) + '...'
      : entry.note

  return (
    <motion.button
      layout
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, x: 100 }}
      transition={{ type: 'spring', stiffness: 500, damping: 35 }}
      onClick={onClick}
      className="w-full text-left p-3 rounded-lg bg-surface hover:bg-surface-2 border border-border-subtle hover:border-border transition-all duration-150 active:scale-[0.98]"
    >
      <div className="flex items-center gap-2 mb-1.5">
        <Badge type={entry.noteType} size="sm" />
        <span className="text-xs text-text-muted ml-auto">
          {formatDate(entry.date)}
        </span>
      </div>
      <p className="text-sm text-text-primary leading-relaxed mb-2">
        {truncatedNote}
      </p>
      <div className="flex items-center gap-1.5 text-[11px] text-text-muted">
        {entry.source && (
          <>
            <span>{entry.source}</span>
            <span>·</span>
          </>
        )}
        <span className="text-text-secondary font-medium">
          {entry.object}
        </span>
      </div>
    </motion.button>
  )
}